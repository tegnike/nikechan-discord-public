...
- 添付画像はDiscordメッセージIDと紐付けて7日間ローカル保持し、現在または後続の画像依頼で実画像参照として画像APIへ渡す
- 添付キャラクターとAIニケちゃんの混合生成では、添付画像と公式三面図の両方を参照する
- 画像生成意図・添付参照・混合生成の意味判定は会話を読むLLMが行い、キーワードや正規表現では分類しない
...
- 一般の参照画像生成は、現在の添付または7日以内に保存したDiscord画像だけを一時トークンで許可し、ユーザー指定パスは受け取らない
- 保存画像は同一チャンネル・同一ユーザーの画像参照発言、元画像への返信、同一チャンネルのDiscordメッセージリンクから復元する
- 他ユーザーの保存画像は、同一チャンネルで元画像へ明示的に返信するかメッセージリンクを指定した場合だけ参照できる
- 混合生成でもDiscord添付画像の一時トークンと公式三面図だけを組み合わせ、任意パスは受け取らない
- pluginは安全な添付画像の一時トークンとツール選択方針だけをLLMへ渡し、依頼の意味はGPT-5.5が会話文脈から判断する
